...
public class UpdateWorkBody {

    @ApiModelProperty(
        value = "(Optional) New ID for this record.",
        example = "Highlight101"
    )
    public String getId() { return null; }

    @ApiModelProperty(
        value = "(Optional) New display name for this record.",
        example = "Highlight 101"
    )
    public String getName() { return null; }

    @ApiModelProperty(
        value = "(Optional) New description for this record (Markdown supported). An embedded `data:` image is rejected — upload the image as an attachment instead.",
    ...
    @ApiModelProperty(
        value = "(Optional) Icon image for this record.",
    ...
    @ApiModelProperty(
        value = "(Optional) Target start date for this record.",
        example = "2024-01-02"
    )
    public String getStart() { return null; }

    @ApiModelProperty(
        value = "(Optional) Target due date for this record.",
        example = "2024-05-25"
    )
    public String getDue() { return null; }

    @ApiModelProperty(
        value = "(Optional) Archive toggle. Specify true to archive this record; specify false to unarchive.",
        example = "true"
    )
