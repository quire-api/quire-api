...
- **Task search:** three new query parameters on `/task/search/...` (#9338, round 2). Each accepts the `,` (AND) / `|` (OR) / `!` (NOT) grammar already used by `assignee` / `tag`.
    ...
    - `?type=` — filter by task type: `normal` (alias `task`), `section`, `milestone`. Useful for structural navigation, e.g., listing only sections in a project. Example: `?type=section|milestone`.
...
- **Task API:** `/task/list/...` gained opt-in pagination via `?limit=N` and `?cursor=<token>` (#24559). Without either param, the response stays a bare full array. With `?limit=N` (1..1000) and more results to come, the **last item in the response carries `"cursor": "<token>"`** — pass it back as `?cursor=<token>` on the next call (along with `?limit=` and any filter like `?status=`) to fetch the next page; end of stream is signalled by the absence of `cursor` on the last item. `?limit=no` is the explicit "return everything". Same shape (bare array) on every page — no envelope, so existing parsers don't need a second branch.
    ...
    - **Mutually exclusive with `?depth>1`** — pick either snapshot mode (subtree fetch) or per-level pagination, not both. Composes orthogonally with `?status=` filter and with `?return=compact` (the latter rendering each item as `{oid, id, cursor?}` for navigation/lookup workloads at minimal token cost).
    - For `/task/list`, the cursor token IS the last task's OID — discoverable from the response without inspection.
- **Task API:** [`GET /task/list/id/{projectId}/{taskId}`](https://quire.io/dev/api/#operation--task-list-id--projectId---taskId--get) gained an opt-in [`?depth=N`](https://quire.io/dev/api/#operation--task-list-id--projectId---taskId--get) parameter for fetching a whole subtree in one call (#24560). Omit (or `?depth=1`) for the flat single-level behavior; any positive `?depth=N` returns a multi-level subtree capped at the given depth; `?depth=full` walks every level (bounded by the per-tier total-nodes cap). Each task carries a nested `tasks` field for its walked children, recursively.
    ...
    - **`?return=compact` composes** — records are rendered as `{oid, id, tasks?, cropped?}` per node; the tree shape is unchanged. The compact + full-depth combination is intended for navigation/lookup workloads where the agent wants the whole subtree's identifier graph at minimal token cost.
    ...
    - The response shape is **identical to the flat `?depth=1` shape** — a bare list of task maps — so existing parsers don't need a second branch for the depth case.
- **Task API:** Added three bulk endpoints for creating, updating, and removing many tasks in one call (up to **300 items per call**, project-scoped, atomic transaction). Body for all three is a top-level JSON array (no envelope); response is a same-length array (one element per submitted item, with `null` placeholders for skipped items in `bulk-update` / `bulk-remove`). Designed for MCP / scripted callers to reduce round trips and token usage; pair with `?return=compact` for large batches.
    ...
    - **Skip-not-found** (`bulk-update` / `bulk-remove` only): items whose target task can't be found (already removed, never existed, cascade-removed by an earlier item in the same batch, etc.) are silently skipped — the corresponding response slot is `null` and the rest of the batch proceeds. Idempotent for agents — safe to re-submit after partial failures or concurrent removals without diffing state first. Shape errors (malformed ref, both / neither `oid` and `id`) still return `400` and roll back.
...
- **Task API:** Added the bulk-move endpoint for repositioning many tasks within a project in one call (#24554). Same body shape, atomic / skip-not-found / `items[i]:` error / sliding-chain / rate-limit conventions as the bulk-add / bulk-update / bulk-remove endpoints.
...
- **Task API:** Added the bulk-transfer endpoint for moving many tasks across projects in one call (#24555). Same body shape, atomic / skip-not-found / `items[i]:` error / sliding-chain / rate-limit conventions as the other bulk endpoints, plus the per-aspect remap flags from single-task transfer.
...
- **All single-item POST / PUT endpoints (MCP):** Added an optional `?return=` query parameter to control the response shape — `full` (default; same as before) returns the complete record, `compact` returns identifiers only. Pass `?return=compact` whenever you don't need the full record back: it cuts the response payload, the caller's parse cost (and LLM-token usage for MCP integrations), AND a server-side reload + render of the full response (per-user lookups, custom-field expansion, etc.). Compact response shape varies by entity:
